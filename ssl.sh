...
addSSlPortToEnv(){
    env_file="/var/www/html/panel/.env"
    key_to_update="PORT_PANEL_SSL"
 
    if grep -q "^$key_to_update=" "$env_file"; then
        # Key exists; update the value
        sed -i "s/^$key_to_update=.*/$key_to_update=$sslPanelPort/" "$env_file"
    else
        # Key doesn't exist; add the key-value pair
        echo "$key_to_update=$sslPanelPort" >> "$env_file"
    fi
}
...
# SSLSessionTickets Off
ENDOFFILE
sed -i "s/PORT_PANEL_SSSL=8081/PORT_PANEL=$panelPort/" /var/www/html/panel/.env
...
addSSlPortToEnv
clear
printf "\nHTTPS Address : https://${domain}:${sslPanelPort}/login \n"